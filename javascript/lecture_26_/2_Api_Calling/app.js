...
//         .then((data)=> data.json())
//         .then((data)=>{
//             return {'lon' : data[0].lon, 'lat': data[0].lat};    // hum return isiliye kar rahe h kyu ki function ka kamm karna hota h hume object bana ke return kiya h
//         })
//         .catch((err)=>{
//             console.log(err);
...
fetch(geoCodingAPI(city_name,API_Key))
    ...
    .then((data)=>{
        getWeatherDeatils(API_Key, data.lat, data.lon);
    })
    .catch((err)=>{
        console.log(err);        
    })